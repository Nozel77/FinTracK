...
export async function POST(request: Request): Promise<Response> {
  try {
    const body = await parseRequestBody(request, parseAddReceivableBody, {
      maxBytes: 32 * 1024,
    });

    const authenticatedUser = await requireAuthenticatedUser(
      "You must be signed in to add a receivable.",
    );
    ...
    const result = await actionService.addReceivable({
      userId,
      debtor: body.debtor,
      amount: body.amount,
      status: body.status,
    ...
    return jsonError("Failed to add receivable.", {
      status: 500,
      code: "INTERNAL_ERROR",
      details: errorToObject(error),
    });
...
function parseAddReceivableBody(value: unknown): AddReceivableRouteBody {
  const payload = assertRecord(value);

  const debtor = getRequiredString(payload, "debtor", 120);
  const amount = getRequiredPositiveNumber(payload, "amount");
  const status = getRequiredPaymentStatus(payload, "status");
...
function getRequiredPaymentStatus(
  payload: Record<string, unknown>,
  fieldName: string,
): "paid" | "unpaid" {
  const value = payload[fieldName];
